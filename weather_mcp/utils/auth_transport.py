...
class AuthenticatedStreamableHttpTransport:
    """
    A wrapper around the streamable-http transport that adds authentication support.
    
    This class wraps the standard streamable-http transport and adds authentication
    before allowing connections to the streamable-http endpoint.
    """
    
    def __init__(self, original_transport, secret_key):
        """
        Initialize the authenticated streamable-http transport.
        
        Args:
            original_transport: The original streamable-http transport to wrap
            secret_key: The secret key used to validate tokens
        """
        self.original_transport = original_transport
        self.secret_key = secret_key
        logger.info("Initialized AuthenticatedStreamableHttpTransport")
    ...
    async def handle_request(self, request):
        ...
        """
        logger.info(f"AuthenticatedStreamableHttpTransport handling request: {request.method} {request.url.path}")
